...
export default function Admin() {
  ...
  const handleExportAsCode = () => {
    // Get all data
    const projectsData = projects.length > 0 ? projects : JSON.parse(localStorage.getItem('portfolioProjects') || '[]');
    const blogsData = blogs.length > 0 ? blogs : JSON.parse(localStorage.getItem('portfolioBlogs') || '[]');
    const achievementsData = achievements.length > 0 ? achievements : JSON.parse(localStorage.getItem('portfolioAchievements') || '[]');
    const certificationsData = certifications.length > 0 ? certifications : JSON.parse(localStorage.getItem('portfolioCertifications') || '[]');
    const volunteeringData = volunteering.length > 0 ? volunteering : JSON.parse(localStorage.getItem('portfolioVolunteering') || '[]');

    // Generate TypeScript files content
    const files = {
      'projects.ts': generateProjectsFile(projectsData),
      'blogs.ts': generateBlogsFile(blogsData),
      'achievements.ts': generateAchievementsFile(achievementsData),
      'certifications.ts': generateCertificationsFile(certificationsData),
      'volunteering.ts': generateVolunteeringFile(volunteeringData),
      'README.md': generateReadmeFile()
    };

    // Create zip-like structure (multiple files)
    ...
    alert('✅ All code files exported! Check your downloads folder.\n\n' +
          'Files downloaded:\n' +
          '- projects.ts\n' +
          '- blogs.ts\n' +
          '- achievements.ts\n' +
          ...
          'Simply replace the files in src/data/ folder!');
  };

  // Helper functions to generate TypeScript files
  const generateProjectsFile = (data: Project[]) => {
    return `// Projects Data - Auto-generated from Admin Panel
...
`;
  ...
  const generateBlogsFile = (data: BlogPost[]) => {
    return `// Blog Posts Data - Auto-generated from Admin Panel
// Last updated: ${new Date().toLocaleString()}
...
export const blogs: BlogPost[] = ${JSON.stringify(data, null, 2)};
...
`;
  ...
  const generateAchievementsFile = (data: Achievement[]) => {
    return `// Achievements Data - Auto-generated from Admin Panel
// Last updated: ${new Date().toLocaleString()}
...
`;
  ...
  const generateCertificationsFile = (data: Certification[]) => {
    return `// Certifications Data - Auto-generated from Admin Panel
// Last updated: ${new Date().toLocaleString()}
...
`;
  ...
  const generateVolunteeringFile = (data: Volunteering[]) => {
    return `// Volunteering Data - Auto-generated from Admin Panel
// Last updated: ${new Date().toLocaleString()}
...
export const volunteeringRoles: Volunteering[] = ${JSON.stringify(data, null, 2)};
`;
  };

  const generateReadmeFile = () => {
    return `# 📦 Exported Portfolio Data Files

## Auto-generated on ${new Date().toLocaleString()}

### 🚀 How to Use These Files:

1. **Replace old files** in your project:
   - Copy \`projects.ts\` → \`src/data/projects.ts\`
   - Copy \`blogs.ts\` → \`src/data/blogs.ts\`
   - Copy \`achievements.ts\` → \`src/data/achievements.ts\`
   - Copy \`certifications.ts\` → \`src/data/certifications.ts\`
   - Copy \`volunteering.ts\` → \`src/data/volunteering.ts\`
...
3. **Done!** Your changes will be live in 2-3 minutes on Vercel.

### 📝 What Changed:
- Projects: ${projects.length} items
- Blogs: ${blogs.length} items
- Achievements: ${achievements.length} items
- Certifications: ${certifications.length} items
- Volunteering: ${volunteering.length} items

### ⚠️ Important Notes:
- These files are ready to use - just copy and replace!
- All TypeScript interfaces are included
- localStorage fallback is preserved
- No manual editing needed

### 🎉 That's it!
Your portfolio content is now ready to deploy!
`;
  };
